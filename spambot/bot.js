...
const input = prompt("What speed? (1-10 smaller = faster)")


const prompt2 = require("prompt-sync")();

const input2 = prompt2("Message?")
  
console.log("Ready to spam");
var maxMessages = 1000000;
var timeToWait = null, minTime = 1000 * input, maxTime = minTime + 1000; //adjusts spamming time (increase to increase delay) Decrease from 1000 to increase speed
var content = null;
var prune = false;
setArgValues();
      ...
      if (command === "spam") {
        function sendSpamMessage() {
          ...
          if (count < maxMessages) {
            ...
            message.channel.send("Spamming Terminated");
            message.channel.send("------------------");
          }
        }
  ...
  for (let j = 2; j < argLength; j++) {
    ...
    if (argsLeft) {
      ...
      if (minTime && maxTime && minTime > maxTime) {
        console.error("Stop messing with the code lmao");
        process.exit();
      }
    }
